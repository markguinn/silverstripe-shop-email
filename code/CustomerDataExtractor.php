...
class CustomerDataExtractor
{
	...
	public function extract($obj) {
		...
		if ($obj->Email) {
			...
			if ($obj->FirstName) $data['FirstName'] = $obj->FirstName;
			if ($obj->Surname) $data['LastName'] = $obj->Surname;
			if ($obj->Company) $data['Company'] = $obj->Company;
		} elseif ($obj->hasMethod('Member') && $member = $obj->Member()) {
			$data['Email'] = $member->Email;
			if ($member->FirstName) $data['FirstName'] = $member->FirstName;
			if ($member->Surname) $data['LastName'] = $member->Surname;
			if ($member->Company) $data['Company'] = $member->Company;
		}
